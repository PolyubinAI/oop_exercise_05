...
template <typename T>
void new_queue<T>::push(T const& el)
{
    try
    {
        if(size_ == cap_)
...
template <typename T>
void new_queue<T>::insert(iterator<T>& iter, T& el)
{
    try
    {
...
template <typename T>
T new_queue<T>::erase(iterator<T>& iter)
{
    try
    {
        if(iter >= end() || iter < begin()) { throw std::exception(); }
...
//Удаление по индексу
template <typename T>
T new_queue<T>::deletion(size_t const& ind)
{
    try
    {
